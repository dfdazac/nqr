...
def write_links(dataset, ent_out, small_ent_out, max_ans_num, name):
    ...
    # return queries, tp_answers, fn_answers, fp_answers
...
def generate_queries(dataset, query_structures, gen_num, max_ans_num, gen_train, gen_valid, gen_test, query_names, save_name):
    ...
    if query_structure == ['e', ['r']]:
        ...
        return ([train_queries, {list2tuple(query_structure): train_tp_answers}, {list2tuple(query_structure): train_fn_answers}],
                [valid_queries, {list2tuple(query_structure): valid_tp_answers}, {list2tuple(query_structure): valid_fn_answers}],
                [test_queries, {list2tuple(query_structure): test_tp_answers}, {list2tuple(query_structure): test_fn_answers}])
...
def main(dataset, seed, gen_train_num, gen_valid_num, gen_test_num, max_ans_num, gen_id, reindex, gen_train, gen_valid, gen_test, save_name, index_only):
    ...
    query_counts = params[query_names[gen_id]]
    # query_counts = [0, 10, 10]
    generate_queries(dataset,
                     query_structures[gen_id:gen_id + 1],
