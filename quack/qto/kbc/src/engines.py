...
def setup_ds(opt):
    dataset_opt = {k: v for k, v in opt.items() if k in ['dataset', 'data_path', 'device', 'cache_eval', 'reciprocal']}
...
class KBCEngine(object):
    ...
    def episode(self):
        ...
        for e in range(init_epoch_id, self.max_epochs):
            ...
            self.model.train()
            pbar = tqdm(total=exp_train_sampler.size, mininterval=5.0, desc=f"Training Epoch {e + 1}")
            ...
            if e % self.opt['valid'] == 0:
                ...
                if res[split]['MRR'] > best_valid_mrr:
                    ...
                    self.model.checkpoint(model_cache_path=self.model_cache_path, epoch_id='model')
